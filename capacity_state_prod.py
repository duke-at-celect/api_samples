...
def get_state(brand):
	...
	cap_state = d2[['country', 'store_id', 'shipments_capacity', 'store_type', 'enable_ship_from_store']]
...
def write_state():
	...
	df[['store_id', 'shipments_capacity']]
...
parser.add_argument('--state', choices=['get', 'set'], help='Either get current state or set state to a previous get.')
parser.add_argument('--region',  choices=['ca', 'us'], help="Required with --state get Must be either 'ca' for Canada or 'us' for United States")
parser.add_argument("--file",  help="Required with --state set. The file to process - include path if necessary")
...
if state == 'get':
	if region == 'ca':
		...
		print('ERROR : missing --region : use --help for more information : Exiting')
		exit()
elif state == 'set':
